<?php
	...
	$ajaxTables = $_POST["tables"];
	$ajaxAttributes = $_POST["attributesToGet"];
	...
	function attributes($tables, $attributesToGet) {
		...
		foreach($tables as $table) {
			...
			//echo $attributesToReturn;
			
			//For each table get the required attributes
			//$ids = "";
			
			//echo $ids;
			
			$thisTablesIDs = $attributesToGet[$counter];
			//echo $attributesToGet[$counter];
			$ids = ""; 
			...
			$attributesSQL = "SELECT * FROM $attributeTable WHERE \"ID\" IN ( $ids ) ;";
